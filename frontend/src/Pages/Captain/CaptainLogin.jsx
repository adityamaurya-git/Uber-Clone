...
export const CaptainLogin = () => {
    ...
    const submitHandler = (e) => {
        ...
        const response = async (formData) =>{
            await instance.post('/api/captain/login' , formData).then((res)=>{
            ...
            <div>
                ...
                <form action="" onSubmit={submitHandler}>
                    ...
                    <input className="bg-[#eeeeee] mb-5 rounded px-4 py-2  w-full text-lg placeholder:text-base"
                        ...
                        type="email" name="" required placeholder="cap@gmail.com" />
                ...
                <p className="text-center">Join a fleet? <Link to='/captain/register' className="text-blue-600 font-semibold" >Register as Captain</Link></p>
            </div>

            <div>
                <Link to='/user/login' className="bg-[#d5622d] flex items-center justify-center text-white font-semibold mb-7 rounded px-4 py-2 w-full text-lg placeholder:text-base">Sign in as User</Link>
            </div>
        </div>
    </>)
}